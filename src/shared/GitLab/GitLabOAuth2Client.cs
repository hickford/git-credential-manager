...
namespace GitLab
{
    public class GitLabOAuth2Client : OAuth2Client
    {
        ...
        internal static string GetClientId(ISettings settings, Uri baseUri)
        {
            ...
            if (GitLabConstants.GitLabInstancesByHost.TryGetValue(baseUri.Host, out instance)) {
                return instance.OAuthClientId;
            }
            throw new ArgumentException($"Missing OAuth configuration for {baseUri.Host}, try setting git config variable {Constants.GitConfiguration.Credential.SectionName}.{GitLabConstants.GitConfiguration.Credential.DevOAuthClientId} ");
        }
        ...
        private static string GetClientSecret(ISettings settings, Uri baseUri)
        {
            ...
            throw new ArgumentException($"Missing OAuth configuration for {baseUri.Host}, try setting git config variable {Constants.GitConfiguration.Credential.SectionName}.{GitLabConstants.GitConfiguration.Credential.DevOAuthClientSecret} ");
        }
    }
}

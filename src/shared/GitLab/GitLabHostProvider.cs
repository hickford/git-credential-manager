...
namespace GitLab
{
    public class GitLabHostProvider : HostProvider
    {
        ...
        private readonly IGitLabRestApi _GitLabApi;
        private readonly IGitLabAuthentication _GitLabAuth;

        public GitLabHostProvider(ICommandContext context)
            : this(context, new GitLabRestApi(context), new GitLabAuthentication(context)) { }

        public GitLabHostProvider(ICommandContext context, IGitLabRestApi GitLabApi, IGitLabAuthentication GitLabAuth)
            : base(context)
        {
            EnsureArgument.NotNull(GitLabApi, nameof(GitLabApi));
        ...
        public override bool IsSupported(InputArguments input)
        {
            ...
            if (GitLabConstants.GitLabInstancesByHost.ContainsKey(hostName))
            {
                return true;
            }
